...
# File to store like count persistently
LIKE_COUNT_FILE = "like_count.json"
...
def save_like_count(count: int, page_id: str = "default") -> bool:
    ...
    try:
        ...
        if response.data:
            ...
            # Insert new record
            client.table('likes').insert({'page_id': page_id, 'count': count}).execute()
...
def get_archive_list():
    ...
    try:
        ...
        archive = [{"page_id": item.get('page_id'),
                    "count": item.get('count'),
                    "title": "Missing Title"} for item in response.data]
        archive_page_ids = [item['page_id'] for item in archive]
        ...
        for archive_dir in os.listdir("static/archive"):
            if archive_dir not in archive_page_ids:
                archive.append({"page_id": archive_dir, "count": 0, "title": "Missing Title"})

        # Sort the archive by page_id
        archive.sort(key=lambda x: x['page_id'], reverse=True)
        logging.info(f"Got archive: {archive}")
        return archive
...
def get_latest_page_id():
    """Get the latest page ID from the archive filesystem"""
    try:
        archive_dirs = os.listdir("static/archive")
        if archive_dirs:
            return sorted(archive_dirs, reverse=True)[0]
        logging.info("No archive directories found.")
        return 'default'
    except Exception as e:
        logging.error(f"Error getting latest page ID: {e}")
        return 'default'
...
def increment_likes():
    ...
    count = get_like_count(page_id) + 1
    save_like_count(count, page_id)
    return jsonify({"count": count})
